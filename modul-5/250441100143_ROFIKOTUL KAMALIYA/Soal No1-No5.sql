...
INSERT INTO mahasiswa VALUES
  ...
  ('25001', 'Indra Mahendra', 2025, 'Tekniik Informatika'),
...
SELECT MIN(sks) AS SKS_Minimal, MAX(sks) AS SKS_Maksimal, AVG(sks) AS Rata_Rata_SKS, COUNT(kode_mk) AS Total_Mata_Kuliah
FROM mata_kuliah;

SELECT m.nim, m.nama, SUM(mk.sks) AS Total_SKS FROM mahasiswa AS m
JOIN krs AS k ON m.nim = k.nim
JOIN mata_kuliah AS mk ON k.kode_mk = mk.kode_mk
WHERE k.semester IN (1, 3)
GROUP BY m.nim, m.nama;

SELECT k.nim, SUM(mk.sks) AS Total_SKS FROM krs AS k
JOIN mata_kuliah AS mk ON k.kode_mk = mk.kode_mk
GROUP BY k.nim
HAVING SUM(mk.sks) > 5;

SELECT sks, COUNT(kode_mk) AS Total_MK, SUM(sks) AS Total_SKS
FROM mata_kuliah
GROUP BY sks
